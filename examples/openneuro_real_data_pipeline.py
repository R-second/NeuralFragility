...
def require_optional_dependencies():
    try:
        ...
        raise SystemExit("Install OpenNeuro extras first: pip install -r requirements-openneuro.txt") from exc
    return mne, pd
...
def detect_seizure_onset(events_file):
    ...
    onset_rows = events[events["trial_type"].str.contains("onset", case=False, na=False)]
    if onset_rows.empty:
...
def resolve_openneuro_files(data_dir, subject_prefix):
    ...
    if subject_prefix == "auto":
        ...
        if not matches:
            raise FileNotFoundError(f"No {subject_prefix}_ieeg.vhdr found under {data_dir}")
        vhdr_file = matches[0]
    ...
    missing = [path for path in [vhdr_file, events_file, channels_file] if not path.exists()]
    if missing:
...
def load_and_preprocess_raw(args):
    ...
    channels_to_drop = [channel for channel in bad_channels if channel in raw_cropped.ch_names]
    if channels_to_drop:
    ...
    if args.highpass_freq > 0:
        ...
        raw_cropped.filter(l_freq=args.highpass_freq, h_freq=None, method="iir", iir_params=iir_params)
...
def main():
    parser = argparse.ArgumentParser(description="Run OpenNeuro BrainVision fragility analysis from local data.")
    parser.add_argument("--data-dir", default="data", help="Local data directory ignored by git.")
    parser.add_argument("--subject-prefix", default=DEFAULT_PREFIX, help="OpenNeuro file prefix, or 'auto'.")
    parser.add_argument("--seconds-before", type=float, default=60.0, help="Seconds before seizure onset.")
    parser.add_argument("--seconds-after", type=float, default=60.0, help="Seconds after seizure onset.")
    ...
    parser.add_argument("--ridge", type=float, default=1e-4, help="Ridge regularization for A estimation.")
    parser.add_argument("--method", choices=["proposed", "grid"], default="proposed", help="Fragility solver.")
    parser.add_argument("--grid-points", type=int, default=100, help="Grid points when method=grid.")
    parser.add_argument("--gamma", type=float, default=0.01, help="Algorithm gamma.")
    parser.add_argument("--max-iter", type=int, default=20, help="Maximum algorithm iterations.")
    parser.add_argument("--epsilon", type=float, default=1e-6, help="Algorithm convergence tolerance.")
    parser.add_argument("--max-windows", type=int, default=5, help="Limit windows for experiments; 0 means all.")
    parser.add_argument("--notch-freq", type=float, default=60.0, help="Notch filter frequency; 0 disables.")
    parser.add_argument("--notch-width", type=float, default=2.0, help="Notch filter width.")
    parser.add_argument("--highpass-freq", type=float, default=0.5, help="High-pass frequency; 0 disables.")
    parser.add_argument("--no-car", action="store_true", help="Disable common average reference.")
    parser.add_argument("--output", default=str(output_path("openneuro_real_heatmap.png")), help="Output heatmap path.")
    parser.add_argument("--data-output", default=str(output_path("openneuro_real_fragility.npz")), help="Output arrays path.")
    args = parser.parse_args()

    raw, onset_time = load_and_preprocess_raw(args)
    windows, window_times = create_sliding_windows(raw, window_size_ms=args.window_ms, step_size_ms=args.step_ms)
    ...
    A_matrices = estimate_linear_models(windows, l2_lambda=args.ridge)
    radii = spectral_radii(A_matrices)
    print(f"Stable A matrices: {np.sum(radii <= 1.0)} / {len(radii)}")
    ...
    plot_heatmap(normalized_fragility, window_times, onset_time, raw.ch_names, args.output)
